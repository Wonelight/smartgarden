"""
Prediction Service — hybrid FAO-56 + ML (XGB/RF) prediction.

Architecture:
  1. FAO-56 forward simulation → fao_pred_24h (deterministic physics baseline)
  2. ML model predicts residual = actual_depletion - fao_pred (data-driven correction)
  3. Final prediction = fao_pred_24h + residual

This hybrid approach lets FAO-56 handle the physics (~80-90% of the signal)
while ML corrects for stochastic rain, sensor drift, and local effects.
...
class PredictionService:
    """ML prediction & training service (XGB primary, RF fallback)."""
    ...
    def predict(self, features_df: pd.DataFrame) -> Tuple[float, float, float]:
        """
        Hybrid prediction: FAO-56 baseline + ML residual correction.

        1. FAO-56 forward sim produces fao_pred_24h (deterministic physics).
        2. ML predicts the residual (what FAO-56 misses: stochastic rain,
           sensor drift, local micro-climate effects).
        3. Final = fao_pred_24h + residual.
